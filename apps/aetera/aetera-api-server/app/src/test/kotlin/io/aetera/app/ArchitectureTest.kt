...
class ArchitectureTest {
    ...
    // ── Aetera 모듈 계약 ──────────────────────────────────────────────────
    // 모듈(schedule, 앞으로 올 budget/guide, ...)은 서로를 몰라야 한다.
    // 새 모듈 도메인을 추가하면 여기 목록에도 한 줄 추가한다.

    @ArchTest
    val aeteraModulesDoNotReferenceEachOther: ArchRule =
        noClasses()
            .that()
            .resideInAPackage("$ROOT_PACKAGE..schedule..")
            .should()
            .dependOnClassesThat()
            .resideInAnyPackage(
                // 아직 없는 미래 모듈들. 모듈 도메인을 추가하면 여기(그리고 그 모듈 쪽 규칙)에 한 줄 추가한다.
                "$ROOT_PACKAGE..budget..",
                "$ROOT_PACKAGE..guide..",
