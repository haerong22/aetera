"use client";

import { useEffect, useState, type FormEvent } from "react";
import { Trash2 } from "lucide-react";
import { Button } from "@/components/ui/Button";
import { Dialog } from "@/components/ui/Dialog";
import { Input } from "@/components/ui/Input";
import { MoneyInput } from "@/components/ui/MoneyInput";
import { Select, optionsFrom } from "@/components/ui/Select";
import {
  useCreateIncome,
  useDeleteIncome,
  useUpdateIncome,
  type Income,
  type IncomeCategory,
  type IncomeCycle,
} from "../api";
import { CATEGORY_LABELS, CYCLE_LABELS } from "../labels";

export function IncomeDialog({
  open,
  onClose,
  income,
}: {
  open: boolean;
  onClose: () => void;
  income?: Income | null;
}) {
  const create = useCreateIncome();
  const update = useUpdateIncome();
  const remove = useDeleteIncome();

  const [title, setTitle] = useState("");
  const [category, setCategory] = useState<IncomeCategory>("SALARY");
  const [amount, setAmount] = useState("");
  const [cycle, setCycle] = useState<IncomeCycle>("MONTHLY");
  const [memo, setMemo] = useState("");
  const [failed, setFailed] = useState(false);

  useEffect(() => {
    if (!open) return;
    setFailed(false);
    setTitle(income?.title ?? "");
    setCategory(income?.category ?? "SALARY");
    setAmount(income ? String(income.amount) : "");
    setCycle(income?.cycle ?? "MONTHLY");
    setMemo(income?.memo ?? "");
  }, [open, income]);

  const busy = create.isPending || update.isPending || remove.isPending;
  const amountValue = Number(amount || "0");
  const ready = title.trim().length > 0 && amountValue > 0;

  function onSubmit(event: FormEvent) {
    event.preventDefault();
    if (!ready) return;
    setFailed(false);

    const input = {
      title: title.trim(),
      category,
      amount: amountValue,
      cycle,
      memo: memo.trim() || undefined,
    };
    const done = { onSuccess: onClose, onError: () => setFailed(true) };

    if (income) update.mutate({ id: income.id, input }, done);
    else create.mutate(input, done);
  }

  return (
    <Dialog open={open} onClose={onClose} title={income ? "소득 수정" : "소득 추가"}>
      <form onSubmit={onSubmit} className="flex flex-col gap-3">
        <Input
          label="이름"
          autoFocus
          value={title}
          maxLength={100}
          placeholder="월급, 블로그 수익, 월세"
          onChange={(event) => setTitle(event.target.value)}
        />

        <MoneyInput label="실수령액" value={amount} onChange={setAmount} />

        <Select
          label="주기"
          value={cycle}
          options={optionsFrom(CYCLE_LABELS)}
          onChange={(event) => setCycle(event.target.value as IncomeCycle)}
        />

        <Select
          label="분류"
          value={category}
          options={optionsFrom(CATEGORY_LABELS)}
          onChange={(event) => setCategory(event.target.value as IncomeCategory)}
        />

        {/* 분류가 계산을 바꾸는 유일한 칸이라, 고른 순간 무슨 뜻인지 그 자리에서 말한다. */}
        <p className="-mt-1 text-[12.5px] text-grey-500">
          {category === "SALARY"
            ? "월급은 일을 그만두면 멈추는 돈으로 봐요. “몇 달 버티나” 계산에서 빠집니다."
            : "그만둬도 이어지는 돈으로 봐요. “몇 달 버티나” 계산에 들어갑니다."}
        </p>

        <Input
          label="메모 (선택)"
          value={memo}
          maxLength={500}
          placeholder="언제 들어오는지, 계약 기간 같은 걸 적어 두세요"
          onChange={(event) => setMemo(event.target.value)}
        />

        {failed && (
          <p role="alert" className="text-[13px] text-danger">
            저장하지 못했어요. 잠시 후 다시 시도해 주세요.
          </p>
        )}

        <div className="mt-2 flex items-center gap-2">
          <Button type="button" variant="ghost" className="flex-1" disabled={busy} onClick={onClose}>
            취소
          </Button>
          <Button type="submit" className="flex-1" disabled={busy || !ready}>
            {busy ? "저장 중" : "저장"}
          </Button>
        </div>

        {income && (
          <button
            type="button"
            disabled={busy}
            onClick={() => remove.mutate(income.id, { onSuccess: onClose, onError: () => setFailed(true) })}
            className="mt-1 inline-flex items-center justify-center gap-1 text-[13px] font-medium text-grey-400 transition-colors hover:text-danger"
          >
            <Trash2 size={14} aria-hidden />
            삭제
          </button>
        )}
      </form>
    </Dialog>
  );
}
